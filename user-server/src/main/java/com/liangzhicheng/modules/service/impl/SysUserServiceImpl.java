...
public class SysUserServiceImpl extends ServiceImpl<ISysUserDao, SysUserEntity> implements ISysUserService {
    ...
    public SysUserLoginVO login(SysUserDTO userDTO, HttpServletRequest request) {
        ...
        try{
            ...
            SysToolUtil.info("user server login result : " + SecurityUtils.getSubject().isAuthenticated());
            //用户信息
            ...
            if(Constants.ONE.equals(user.getIsAdmin())){
                userVO.setPermMenuList(CacheUtil.listPermMenu());
            }else{
...
//        SysUserEntity user = ContextUtil.getCurrentUser(request);
...
//        SysUserEntity user = ContextUtil.getCurrentUser();
...
//        SysUserEntity user = ContextUtil.getCurrentUser();
    ...
    public IPage listAccount(SysUserDTO userDTO) {
        ...
        //1.账号信息查询
        LambdaQueryWrapper<SysUserEntity> wrapperUser = new LambdaQueryWrapper<SysUserEntity>();
        if(SysToolUtil.isNotBlank(keyword)){
            wrapperUser.and(Wrapper -> Wrapper.like(SysUserEntity::getId, keyword)
                    .or().like(SysUserEntity::getAccountName, keyword));
        }
        wrapperUser.eq(SysToolUtil.isNotBlank(deptId), SysUserEntity::getDeptId, deptId);
        wrapperUser.eq(SysToolUtil.isNotBlank(loginStatus)
                && SysToolUtil.in(loginStatus, Constants.ZERO, Constants.ONE),
                SysUserEntity::getLoginStatus, loginStatus);
        List<SysUserEntity> userList = baseMapper.selectList(
                wrapperUser.eq(SysUserEntity::getDelFlag, Constants.ZERO));
        ...
        //2.账号角色信息查询
        LambdaQueryWrapper<SysRoleUserEntity> wrapperRoleUser = new LambdaQueryWrapper<SysRoleUserEntity>();
        if(SysToolUtil.inOneByNotBlank(roleId)){
            wrapperRoleUser.eq(SysToolUtil.isNotBlank(roleId), SysRoleUserEntity::getRoleId, roleId);
            List<SysRoleUserEntity> roleUserList = roleUserService.list(
                    wrapperRoleUser.eq(SysRoleUserEntity::getDelFlag, Constants.ZERO));
            //账号角色处理
        ...
        if(SysToolUtil.listSizeGT(accountIdByUser)){
            resultList = baseMapper.selectPage(PageQuery.queryDispose(userDTO),
                    new LambdaQueryWrapper<SysUserEntity>().in(SysUserEntity::getId, accountIdByUser)
                            .orderByDesc(SysUserEntity::getLoginStatus).orderByAsc(SysUserEntity::getId));
    ...
    public void saveAccount(SysUserDTO userDTO) {
        ...
        if(SysToolUtil.isNotBlank(roleId)){
            ...
            if(arrayRoleId != null && arrayRoleId.length > 0){
                ...
                roleUserService.update(roleUser, new LambdaQueryWrapper<SysRoleUserEntity>()
                        .eq(SysRoleUserEntity::getAccountId, user.getId())
                        .eq(SysRoleUserEntity::getDelFlag, Constants.ZERO));
...
//        //缓存中权限菜单
//        List<SysMenuVO> cacheListPermMenu = CacheUtil.listPermMenu();
...
//    }
    ...
    private List<SysMenuVO> listPermMenu(String accountId){
        ...
        //缓存中获取所有菜单
        List<SysMenuVO> menuList = CacheUtil.listPermMenu();
